...
$stmt2->execute();
$rawSizeItems = $stmt2->fetchAll(PDO::FETCH_ASSOC);

/* 重複完全除去 */
$uniqueSizes = [];

foreach ($rawSizeItems as $item) {

    // 例：26 → 26、26A → 26.5
    $converted = convertSize($item["size"]);

    // convertSize後の値をキーにして重複排除
    if (!isset($uniqueSizes[$converted])) {
        $uniqueSizes[$converted] = $item;
    }
}

/* サイズ昇順に並び替え */
ksort($uniqueSizes);
$sizeItems = array_values($uniqueSizes);
